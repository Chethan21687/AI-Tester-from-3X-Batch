"""
Standalone locator extractor for amazon.in (busy page — never reaches networkidle,
so this uses 'load' + a short settle pause instead of waiting for network idle).

Usage: python amazon_locators.py [output.md]
"""
import sys
sys.stdout.reconfigure(encoding="utf-8")  # amazon.in renders rupee symbols etc — cp1252 console can't print them
...
def extract():
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_timeout(4000)  # let above-the-fold widgets settle — amazon never hits networkidle
        ...
        for handle in page.query_selector_all(INTERACTIVE_SELECTOR):
            try:
                ...
                continue  # SVG / detached / non-HTMLElement nodes — skip rather than crash the whole pass
...
def main():
    ...
    if xpath_rows:
        print(f"\n{len(xpath_rows)} element(s) needed an XPath fallback (no stable role/label/test-id/CSS hook):")
