...
import { generateTestCases } from './groqClient.js';
...
app.post('/api/cases/generate', async (req, res) => {
  ...
  try {
    ...
    const casesMarkdown = await generateTestCases({
      apiKey: settings.groqApiKey,
      model: settings.groqModel,
      ticket
    });

    res.json({ ticket, casesMarkdown });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.listen(PORT, () => {
  console.log(`Test Case backend running on http://localhost:${PORT}`);
});
