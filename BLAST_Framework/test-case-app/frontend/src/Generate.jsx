...
export default function Generate() {
  ...
  const handleGenerate = async (e) => {
    ...
    setResult(null);
    try {
      const data = await generateTestCases(ticketId.trim() || undefined);
      setResult(data);
    } catch (err) {
  ...
  const handleDownload = () => {
    if (!result) return;
    const blob = new Blob([result.casesMarkdown], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `test_cases_${result.ticket.key}.md`;
    a.click();
  ...
  return (
    <div className="generate">
      <div className="panel-heading">
        <h2><Sparkles size={20} className="accent" /> Generate Test Cases</h2>
      </div>
      <p className="hint">
        Pulls the ticket straight from Jira, hands it to Groq's LLM, and returns a ready-to-use
        Markdown table of detailed test cases — functional, negative, boundary, security and all.
      </p>
      ...
      {loading && (
        <div className="loading-block loading-strategy">
          ...
          <div>
            ...
            <p>Fetching ticket details, then generating your test cases. This can take ~10–20s.</p>
          </div>
        </div>
      )}
